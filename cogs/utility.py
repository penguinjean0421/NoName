...
class Utility(commands.Cog):
    ...
    @app_commands.command(name="choose", description="선택의 고민을 대신 해드립니다.")
    @app_commands.describe(options="선택지들을 공백으로 구분해서 입력해주세요. (예시: 짜장면 짬뽕)")
    ...
    @app_commands.command(name="menu", description="오늘 뭐 먹을지 고민될 때 사용하세요!")
    @app_commands.describe(category="한식, 중식, 일식, 양식, 분식 등 카테고리 선택")
    async def recommend_menu(self, interaction: discord.Interaction, category: str = None):
        target_list = None
        display_category = ""
        ...
        embed.set_footer(
            text=f"팁: /menu [카테고리]를 입력해서 특정 종류만 추천받을 수 있습니다!"
        )
        await interaction.response.send_message(embed=embed)
